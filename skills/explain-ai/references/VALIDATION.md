...
Run `npm run test:e2e` for the tracked browser fixture. It covers catalog → level → subject → lesson routing, deep links, forward/reverse navigation, quiz feedback, repeated lesson entry, desktop canvas/ScrollTrigger initialization and the sub-768px blocking view. The fixture uses semantic state markers rather than animated pixel snapshots; the runtime probe is the rendered Three.js/GSAP evidence.
