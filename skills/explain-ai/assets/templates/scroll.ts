...
export function connectLessonScroll(controller: LessonController, sections: HTMLElement[]) {
  ...
  media.add({ reduced: '(prefers-reduced-motion: reduce)', regular: '(prefers-reduced-motion: no-preference)' }, context => {
    ...
    sections.forEach((section, index) => {
      const sample = (trigger: ScrollTrigger) => controller.showStep(index, trigger.progress, reduced);
      ScrollTrigger.create({
        trigger: section, start: 'top center', end: 'bottom center',
        onUpdate: trigger => { if (trigger.isActive) sample(trigger); },
        onEnter: sample, onEnterBack: sample,
        onLeave: () => controller.showStep(index, 1, reduced),
        onLeaveBack: () => controller.showStep(Math.max(0, index - 1), 1, reduced),
      });
    });
  });
  return () => { media.revert(); controller.dispose(); };
}
