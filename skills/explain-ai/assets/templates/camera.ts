...
export type BoundsCameraMode =
  "wide" | "medium" | "close" | "macro" | "top" | "side" | "best";
/** Fit the bounding sphere against the smaller horizontal/vertical field of view. */
export function frameBounds(
  bounds: Box3,
  camera: PerspectiveCamera,
  mode: BoundsCameraMode = "best",
) {
  if (
    bounds.isEmpty() ||
    camera.aspect <= 0 ||
    ...
    throw new Error("Invalid bounds or camera");
  ...
  const direction =
    mode === "top"
      ...
      : mode === "side"
        ? new Vector3(1, 0, 0)
        : camera.position.clone().sub(sphere.center);
  ...
  return {
    ...
    near: Math.max(0.001, (distance - sphere.radius) / 10),
    far: Math.max(100, distance + sphere.radius * 4),
  };
}
