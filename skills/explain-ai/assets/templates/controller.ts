...
export type ActionHandlers = {
  [K in Action['action']]?: (action: Extract<Action, { action: K }>, progress: number) => void;
};
...
export function createLessonController(lesson: Lesson, design: Design, adapter: SceneAdapter) {
  const weights = lesson.steps.map(step => step.scrollUnits ?? design.motion.scrollUnits);
  if (!weights.length || weights.some(w => !Number.isFinite(w) || w <= 0)) throw new Error('Invalid step pacing');
  for (const step of lesson.steps) for (const action of step.actions) {
    if (!adapter.handlers[action.action]) throw new Error(`Missing action handler: ${action.action}`);
  }
  ...
  function showStep(index: number, progress = 1, reducedMotion = false) {
    ...
    if (!Number.isInteger(index) || !step || !Number.isFinite(progress)) throw new Error('Invalid step or progress');
    ...
    for (const action of step.actions) {
      const handler = adapter.handlers[action.action] as (action: Action, progress: number) => void;
      handler(action, t);
    }
    adapter.render(step);
  ...
  function seek(progress: number, reducedMotion = false) {
    ...
    for (let i = 0; i < weights.length; i++) {
      if (distance < weights[i] || i === weights.length - 1) return showStep(i, distance / weights[i], reducedMotion);
      distance -= weights[i];
    }
    throw new Error('No lesson steps');
  }
  return { showStep, seek, dispose() { if (!disposed) { disposed = true; adapter.dispose(); } } };
}
