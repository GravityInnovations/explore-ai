...
`lesson.d.ts` exports `Lesson`, `Step`, `Action` and action-specific types. Other files export project configuration, design, asset-index and runtime shapes. TypeScript describes structure; numeric bounds, unique IDs, semantic references and filesystem rules still require validation. The declarations contain no executable renderer or default theme.
