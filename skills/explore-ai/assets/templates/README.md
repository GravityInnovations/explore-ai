...
| `CatalogRoutes.ts` | Canonical Home → Catalog → level → subject → lesson route model from catalog entries | Render every surface through the target project's existing shell; keep `/` as Home and never replace it with a lesson or catalog |
...
After copying templates into a target project, adjust relative type imports. Keep filesystem modules on the server and scene/GSAP code behind a client boundary. Render the route model as separate `/`, `/catalog`, `/catalog/<level>`, `/catalog/<level>/<subject>` and `/lesson/<level>/<subject>/<slug>` surfaces. An empty catalog must render a useful empty state without fabricated entries. Templates do not implement all nineteen action effects; do not advertise unsupported geometry in the runtime manifest.
