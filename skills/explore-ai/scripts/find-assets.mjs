...
export async function findAssets(root, query = "") {
  ...
  for (const asset of index.assets) {
    const search = [asset.id, asset.description, ...asset.tags]
      .join(" ")
      .toLocaleLowerCase("en");
    if (terms.every((term) => search.includes(term)))
      matches.push({
        ...asset,
        resolvedPath: await resolveLocal(
          root,
          `${config.paths.assetLibrary}/${asset.path}`,
          { file: true },
        ),
      });
  }
...
if (
  ...
  try {
    if (!process.argv[2])
      throw new Error("Usage: find-assets.mjs <project-root> [search terms]");
    console.log(
      JSON.stringify(
        await findAssets(process.argv[2], process.argv.slice(3).join(" ")),
        null,
        2,
      ),
    );
