...
test('installation is self-contained, excludes dependencies and refuses replacement', async () => {
  ...
  try {
    ...
    assert.match(await readFile(path.join(installed, 'SKILL.md'), 'utf8'), /name: explain-ai/);
    assert.match(await readFile(path.join(installed, 'package-lock.json'), 'utf8'), /ajv/);
    assert.equal(await readFile(path.join(installed, 'LICENSE'), 'utf8'), await readFile(new URL('../LICENSE', import.meta.url), 'utf8'));
