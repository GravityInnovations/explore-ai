...
function opacityFixture(){const f=fixture();f.runtime.actions.push('fade','xray','assemble');f.runtime.components[0].actions.push('fade','xray','assemble');return f;}
test('ordered fade restores opacity without changing visibility',()=>{const f=opacityFixture();f.lesson.steps[0].actions=[{action:'fade',target:'shape',opacity:0},{action:'fade',target:'shape',opacity:1},{action:'highlight',target:'shape.part'}];assert.deepEqual(check(f),[]);f.lesson.steps[0].actions.unshift({action:'hide',target:'shape'});assert.match(JSON.stringify(check(f)),/hidden/);});
test('reveal does not silently undo zero material opacity',()=>{const f=opacityFixture();f.lesson.steps[0].actions=[{action:'fade',target:'shape.part',opacity:0},{action:'reveal',target:'shape.part'},{action:'highlight',target:'shape.part'}];assert.match(JSON.stringify(check(f)),/hidden/);});
test('xray can expose children without making them invisible',()=>{const f=opacityFixture();f.lesson.steps[0].actions.unshift({action:'xray',target:'shape',opacity:0});assert.deepEqual(check(f),[]);});
test('assembly restores baseline state but respects hidden ancestors',()=>{const f=opacityFixture();f.lesson.steps[0].actions=[{action:'fade',target:'shape.part',opacity:0},{action:'hide',target:'shape.part'},{action:'assemble',target:'shape.part'},{action:'highlight',target:'shape.part'}];assert.deepEqual(check(f),[]);f.lesson.steps[0].actions.unshift({action:'hide',target:'shape'});assert.match(JSON.stringify(check(f)),/hidden/);});
