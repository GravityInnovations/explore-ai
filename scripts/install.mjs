...
const within = (root, child) => { const rel = path.relative(root, child); return rel === '' || (!rel.startsWith(`..${path.sep}`) && rel !== '..' && !path.isAbsolute(rel)); };

export async function install(project) {
  const root = await realpath(path.resolve(project));
  if (!(await lstat(root)).isDirectory()) throw new Error('Target project must be a directory');
  let parent = root;
  for (const segment of ['.agents', 'skills']) {
    parent = path.join(parent, segment);
    try { await mkdir(parent); } catch (error) { if (error.code !== 'EEXIST') throw error; }
    if (!within(root, await realpath(parent))) throw new Error('Installation directory escapes target project');
  }
  ...
  await cp(source, destination, {
    ...
    filter: async (entry) => {
      ...
      if ((await lstat(entry)).isSymbolicLink()) throw new Error(`Package symlinks are not supported: ${entry}`);
      return true;
    },
  });
  return destination;
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  try {
    if (process.argv.length !== 3) throw new Error('Usage: node scripts/install.mjs <existing-target-project>');
